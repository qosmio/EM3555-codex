...
bool emAfPreZDOMessageReceived(EmberNodeId emberNodeId,
...
// ******************************************************************
uint16_t joined_dev_addr[40] = {0,};
uint8_t joined_num = 0;
                    ...
                    uint16_t length)
{
  ...
  switch (apsFrame->clusterId) {
    ...
    {
        emberAfZdoPrintln("Device Announce: 0x%2x", (uint16_t)(message[1]) + (uint16_t)(message[2] << 8));
        joined_dev_addr[joined_num++] = (uint16_t)(message[1]) + (uint16_t)(message[2] << 8);
        if(joined_num >= 40)
        {
            joined_num = 0;
        }
