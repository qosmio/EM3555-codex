...
static void steeringSerialLog(const char *format, ...)
{
  va_list args;
...
static uint8_t currentScanBeaconCount = 0;
static uint8_t currentScanCandidateCount = 0;
...
void gotoNextChannel(void)
{
  ...
  currentScanBeaconCount = 0;
  currentScanCandidateCount = 0;
...
void tryToJoinNetwork(void)
{
  ...
  if (networkParams.panId == EMBER_AF_INVALID_PAN_ID) {
    steeringSerialLog("steer no-candidate ch=%d",
                      emAfPluginNetworkSteeringCurrentChannel);
    gotoNextChannel();
    return;
  }

  steeringSerialLog("steer join pan=0x%2x ch=%d node=%d",
                    networkParams.panId,
                    emAfPluginNetworkSteeringCurrentChannel,
                    emberAfPluginNetworkSteeringGetNodeTypeCallback(emAfPluginNetworkSteeringState));
...
{
  steeringSerialLog("steer scan-complete ch=%d status=%x beacons=%d candidates=%d",
                    channel,
                    status,
                    currentScanBeaconCount,
                    currentScanCandidateCount);
                                 ...
                                 int8_t rssi)
{
  ...
  if (!(networkFound->allowingJoin
    ...
    steeringSerialLog("steer reject pan=0x%2x ch=%d join=%d profile=%d req=%d lqi=%d rssi=%d",
                      networkFound->panId,
                      networkFound->channel,
                      networkFound->allowingJoin ? 1 : 0,
                      networkFound->stackProfile,
                      REQUIRED_STACK_PROFILE,
                      lqi,
                      rssi);
    return;
  }
  ...
  if (gFilterByExtendedPanId) {
    if (0 != MEMCOMPARE(networkFound->extendedPanId,
      ...
      steeringSerialLog("steer reject-xpan pan=0x%2x ch=%d",
                        networkFound->panId,
                        networkFound->channel);
  ...
  if (!addPanIdCandidate(networkFound->panId)) {
    steeringSerialLog("steer add-candidate failed");
    cleanupAndStop(EMBER_ERR_FATAL);
    return;
  }

  currentScanCandidateCount++;
  steeringSerialLog("steer candidate pan=0x%2x ch=%d",
                    networkFound->panId,
                    networkFound->channel);
}
